...
var Widget = Base.extend({
  ...
  delegateEvents: function(element, events, handler) {
    ...
    function handleEvents(handler, widget, args) {
      ...
      // delegate
      if (args.selector) {
        $(element).on(args.type, args.selector, callback);
      }
      // normal bind
      // 分开写是为了兼容 zepto，zepto 的判断不如 jquery 强劲有力
      else {
        $(element).on(args.type, callback);
      }
    }
    ...
    if (argus.length === 0) {
      ...
      if (this._delegateElements) {
        for (var de in this._delegateElements) {
          this._delegateElements.hasOwnProperty(de) &&
